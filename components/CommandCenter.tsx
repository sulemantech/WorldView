...
import { IconLayer, ScatterplotLayer, PolygonLayer, TextLayer } from "@deck.gl/layers";
...
function icon(paths: string, size = 80, vb = "0 0 64 64"): IconDef {
...
const ICONS: Record<string, IconDef> = {
  ...
  // ─── COMMERCIAL AIRLINER ──────────────────────────────────────────────────
  // Top-down view: long fuselage, wide swept-back wings, small T-tail.
  commercial: icon(`
    <ellipse cx="32" cy="32" rx="5" ry="26" fill="white"/>
    <ellipse cx="32" cy="9"  rx="4" ry="5.5" fill="white"/>
    <polygon points="27,21 1,46  5,52 27,35"  fill="white"/>
    <polygon points="37,21 63,46 59,52 37,35" fill="white"/>
    <polygon points="27,52 13,59 15,63 27,56" fill="white"/>
    <polygon points="37,52 51,59 49,63 37,56" fill="white"/>
    <rect x="30" y="53" width="4" height="11" rx="2" fill="white"/>
  `),

  // ─── MILITARY FIGHTER JET ─────────────────────────────────────────────────
  // Delta wing, sharp pointed nose, twin engine exhausts at rear.
  military: icon(`
    <ellipse cx="32" cy="27" rx="3.5" ry="22" fill="white"/>
    <ellipse cx="32" cy="6"  rx="2.5" ry="5"  fill="white"/>
    <polygon points="29,18 1,58 27,50"  fill="white"/>
    <polygon points="35,18 63,58 37,50" fill="white"/>
    <ellipse cx="26" cy="53" rx="5" ry="6" fill="white"/>
    <ellipse cx="38" cy="53" rx="5" ry="6" fill="white"/>
    <polygon points="30,24 14,30 16,34 30,29" fill="white"/>
    <polygon points="34,24 50,30 48,34 34,29" fill="white"/>
  `),
  ...
  carrier: icon(`
    <path fill-rule="evenodd" fill="white" d="
      M8,14 L56,14 L61,21 L57,58 L8,58 L3,21 Z
      M10,44 L50,30 L50,33 L10,47 Z
    "/>
    <rect x="47" y="5"  width="15" height="24" rx="2" fill="white"/>
    <rect x="53" y="1"  width="2.5" height="8" fill="white"/>
    <rect x="57" y="1"  width="2.5" height="8" fill="white"/>
    <rect x="50" y="5"  width="3"   height="9" fill="white"/>
    <ellipse cx="55" cy="28" rx="6" ry="2" fill="white"/>
  `),
  ...
  satellite: icon(`
    ...
    <path fill-rule="evenodd" fill="white" d="
      M2,25  L20,25 L20,39 L2,39  Z
      M5,27  L7,27  L7,37  L5,37  Z
      M9,27  L11,27 L11,37 L9,37  Z
      M13,27 L15,27 L15,37 L13,37 Z
      M17,27 L19,27 L19,37 L17,37 Z
    "/>
    <path fill-rule="evenodd" fill="white" d="
      M44,25 L62,25 L62,39 L44,39 Z
      M45,27 L47,27 L47,37 L45,37 Z
      M49,27 L51,27 L51,37 L49,37 Z
      M53,27 L55,27 L55,37 L53,37 Z
      M57,27 L59,27 L59,37 L57,37 Z
    "/>
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function DeckGLOverlay(props: { layers: any[]; onHover: any; onClick: any }) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export default function CommandCenter({
    ...
    if (vis.noFlyZones) {
      ls.push(
        new PolygonLayer({
          ...
          getFillColor: (d: NoFlyZone): RGBA => d.active ? [255, 51, 102, 35] : [255, 140, 0, 18],
          getLineColor: (d: NoFlyZone): RGBA => d.active ? [255, 51, 102, 200] : [255, 140, 0, 110],
  ...
  const zoomIn     = () => m()?.zoomIn({ duration: 300 });
  const zoomOut    = () => m()?.zoomOut({ duration: 300 });
  const resetView  = () => m()?.flyTo({ center: [44.65, 30.32], zoom: 5.5, pitch: 45, bearing: 0, duration: 1500 });
  const resetNorth = () => m()?.easeTo({ bearing: 0, duration: 800 });
  const toggleTilt = () => { const map = m(); if (map) map.easeTo({ pitch: map.getPitch() > 10 ? 0 : 45, duration: 600 }); };

  const ctrlBtns = [
    { icon: "+",  fn: zoomIn,     tip: "Zoom in"     },
    { icon: "−",  fn: zoomOut,    tip: "Zoom out"    },
    { icon: "⌖",  fn: resetView,  tip: "Reset view"  },
    { icon: "N",  fn: resetNorth, tip: "Reset north" },
    { icon: "3D", fn: toggleTilt, tip: "Toggle tilt" },
  ];
